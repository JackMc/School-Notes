...
void readData()
{
  ...
  while (lineIndex < lines.length)
  {
    ...
    countryData[countryNum].quarters = new int[4];
    int j = 0;
    while (j < 4) {
      // Elements 1...4 inclusive are the quarters
      countryData[countryNum].quarters[j] = Integer.parseInt(splitLine[j+1]);
    }
...
void drawLineGraph(int index)
{
  ...
  final int maxValue = max(countryData[index].quarters);
  ...
  // Objects: We loop over the whole array to get the data points
  float lastPointX = axisX;
  float lastPointY = axisY;

  int pointNum = 0;
  while (pointNum < numPoints) {
    float yValue = countryData[index].quarters[pointNum];
